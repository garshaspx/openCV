...
def start():           #main func to start the program and start window
    ...
    def start_match():        #starting the main prosec
        ...
        while True: #main loop 
            _, frame = cap.read()
            results = model.track(frame, persist=True, conf=info[3], device=0)#proccessing the frame    , device="GPU"          #  save_txt=True save data in txt             , device=[2]
            result = results[0] 
...
def train():    #creating tkinter window to train a new data-set
    ...
    Button(train_win, text="start training", command= lambda : trainer.start()).place(x=200, y=80)
