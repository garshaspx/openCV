...
def cam_finder():  # func to list the cameras
    index = 0
    # accessing the camera connected to  the systemprint('ddddddddd')
    while True:
...
def yol():  # fun to load ultralytics library
    try:
        # tkinter label so we know the is loaded
        load_label = Label(
            win, text="loading... please wait", bg='red', fg='white')
        load_label.place(x=240, y=228)
    except:
        messagebox.showerror(
            "error", "unexpected error happend, please restart the app")
        close(True)
    global YOLO
    from ultralytics import YOLO
    Button(win, text="start", command=lambda: start(),
...
try:  # creating setting table if doesnt exist
    connection.execute(''' CREATE TABLE \"setting\"
            ...
            ''')
    connection.execute(
        f"INSERT INTO setting values (\"{info[0]}\", \"{info[1]}\", \"{info[2]}\", \"{info[3]}\", \"{info[4]}\", \"{info[5]}\", \"{info[6]}\")")
    connection.commit()  # inserting data to setting table if didnt exist and comminting
except:
    # if table does exist reads data
...
def library():  # function to manage library manager window
    ...
    tv = ttk.Treeview(lib_win, columns="number",
                      height=9)          # all window options
    tv.place(x=0, y=0)                                              #
    ...
    def add_direc_txt():  # window to add new library
        ...
        Label(new_library_win, text="enter information for new library :").grid(
            row=0, column=1)
        ...
        Button(new_library_win, text="choose:",
               command=lambda: choose_direc()).grid(row=2, column=1)
        address = "---"

        def choose_direc():
            nonlocal address
            file = filedialog.askopenfile(
                mode='r', filetypes=[('data Files', '*.pt')])
            if file:
        ...
        def save():  # saaving the data in txt file and closing new lib window
            ...
            win.bind('<FocusIn>', win.lower())  # puting the main window behind
        Button(new_library_win, text="save new data",
               command=lambda: save()).grid(row=3, column=3)
    ...
    def active():  # function to make a specific library active
        ...
        info[0] = kk[ac]
        lib_win.destroy()
    Button(lib_win, text="add library",
           command=lambda: add_direc_txt()).place(x=40, y=220)
    Button(lib_win, text="delete library",
           command=lambda: delete()).place(x=135, y=220)
    Button(lib_win, text="active library",
           command=lambda: active()).place(x=240, y=220)
...
def video():  # func to choose video input
    ...
    cam_chooser = ttk.Combobox(
        vid_win, width=20, textvariable=StringVar())  # creating a combobox
    ...
    Button(vid_win, text="save", fg="red",
           command=lambda: save()).place(x=270, y=50)

    def video_loc():  # choosing video file from hard drive
        info[1] = filedialog.askopenfile(
            filetypes=[('data Files', '*.mp4')]).name  # file has to be mp4
        if info[1][-3:] == "mp4":
            vid_win.destroy()
        else:
            messagebox.showerror(
                "file type", "make sure the file you choose is a mp4")
            # win.bind('<FocusIn>', win.lower())
...
def start():  # main func to start the program and start window
    ...
    if info[0] == "None" or info[0] == '----':  # incase input and data-set wasnt choosen
        messagebox.showerror(
            "library error", "choose your library before starting")
        return
    elif info[1] == False:
    ...
    Button(win_start, text="start", command=lambda: Thread(
        target=start_match).start()).place(x=100, y=20)
    Label(win_start, text="stop matching :   press Esc on your keyboard").place(
        x=10, y=90)
    ...
    def switch():  # option to make view mode on or off
        if info[2] == "ON":
            info[2] = "OFF"
            switch_but = Button(
                win_start, text=info[2], command=lambda: switch())
    ...
    def start_match():  # starting the main prosec
        ...
        while True:  # main loop
            _, frame = cap.read()
            # proccessing the frame     , device="GPU"         #  save_txt=True save data in txt             , device=[2]
            results = model.track(frame, persist=True, conf=info[3])
            result = results[0]
            ...
            for box in result.boxes:         # puting bouding box around found items
                ...
                try:   # storing data in database and saving image and labels for training
                    connection.execute(
                        f"INSERT INTO \"data_center\" values (\"{uuid+str(box.id[0].item())}\", \"{class_id}\", {conf}, \"{cords}\", \"{time()}\")")
                    connection.commit()
                except:
                    pass
                ...
                if info[6] == "ON":
                    if i % 20 == 0:
                        imwrite(
                            f"{home}ML_train/train/images/{time()}_{class_id}.jpg", frame)
                        open(f"{home}ML_train/train/labels/{time()}_{class_id}.txt", "w+").write(
                            f"{int(id_item)} {((cords[0]+cords[2])/2/frame.shape[1])} {((cords[1]+cords[3])/2/frame.shape[0])} {(cords[2]-cords[0])/frame.shape[1]} {(cords[3]-cords[1])/frame.shape[0]}")  # x center y center width hight
                    elif i % 61 == 0:
                        imwrite(
                            f"{home}ML_train/valid/images/{time()}_{class_id}.jpg", frame)
                        open(f"{home}ML_train/valid/labels/{time()}_{class_id}.txt", "w+").write(
                            f"{int(id_item)} {((cords[0]+cords[2])/2/frame.shape[1])} {((cords[1]+cords[3])/2/frame.shape[0])} {(cords[2]-cords[0])/frame.shape[1]} {(cords[3]-cords[1])/frame.shape[0]}")  # x center y center width hight
                        i = 0
                    i += 1
...
def train():  # creating tkinter window to train a new data-set
    ...
    def choose_direc():  # loading training algif_playerritm
        nonlocal address
        file_ad = filedialog.askopenfile(
            mode='r', filetypes=[('data Files', '*.pt')])
        if file_ad:
            address = path.abspath(file_ad.name)
            Label(train_win, text=f"chosen:{address}", fg="red").place(
                x=10, y=35)
        win.bind('<FocusIn>', win.lower())
    ...
    def start_train():
        ...
        try:
            ...
            messagebox.showerror(
                "training error", "make sure all data is correct")

    Label(train_win, text="choose algif_playerritm to train model :").place(x=10, y=10)
    Button(train_win, text="choose:",
           command=lambda: choose_direc()).place(x=210, y=10)
    trainer = Thread(target=start_train)
    Button(train_win, text="start training",
           command=lambda: trainer.start()).place(x=200, y=80)
    Button(train_win, text="stop training", bg="red",
           command=lambda: stop_tain()).place(x=100, y=80)

    def stop_tain():
        trainer.raise_exception(SystemExit)
...
def setting():
    ...
    def change_thresh():
        try:
            ...
            messagebox.showerror(
                "threshold error", "threshold must be between 0.1 and 1")
            win.bind('<FocusIn>', win.lower())
    ...
    def switch():
        if info[4] == "ON":
            info[4] = "OFF"
            switch_but_gif = Button(
                s_win, text=info[4], command=lambda: switch())
            switch_but_gif.place(x=140, y=35)
            ...
            Button(win, text="start", command=lambda: start(),
                   fg="blue").place(x=50, y=50)
            Button(win, text="choose input",
                   command=lambda: video()).place(x=50, y=85)
            Button(win, text="library manager",
                   command=lambda: library()).place(x=50, y=120)
            Button(win, text="ML trainer",
                   command=lambda: train()).place(x=50, y=155)
            Button(win, text="setting",
                   command=lambda: setting()).place(x=50, y=190)
            Button(win, text="close", command=lambda: close(
                True), fg="red").place(x=50, y=225)
            s_win.update()
            close(False)
        else:
            info[4] = "ON"
    ...
    def image_for_video():
        if info[5] == "OFF":
            try:
                ...
                mkdir(home+"imgs_for_vid")
            info[5] = "ON"
            switch_but_img = Button(
                s_win, text=info[5]+" ", command=lambda: image_for_video())
    ...
    def data_ml():
        if info[6] == "OFF":
            info[6] = "ON"
            switch_but_ml = Button(
                s_win, text=info[6]+" ", command=lambda: data_ml())
